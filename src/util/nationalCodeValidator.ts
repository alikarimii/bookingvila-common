export const nationalCodeValidator = (value: string, helpers: any) => {
    ...
    if (L < 8 || parseInt(value, 10) == 0) throw new Error('lenght must be 10');
    value = ('0000' + value).substr(L + 4 - 10);
    if (parseInt(value.substr(3, 6), 10) == 0) throw new Error('code not valid');
    var c = parseInt(value.substr(9, 1), 10);
    var s = 0;
    for (var i = 0; i < 9; i++)
        s += parseInt(value.substr(i, 1), 10) * (10 - i);
    s = s % 11;
    if ((s < 2 && c == s) || (s >= 2 && c == (11 - s))) return value;
